...
- **Redundant Database Logic**: There are several `_temp` files (e.g., `src/lib/db/leagues.ts_temp`) and overlapping logic between `src/lib/db` and `src/lib/services`. These need consolidation.
...
- **League Management**: Logic for "switching perspective" exists in both the `PerspectiveContext` and partially in the `Chat API` route. This should be unified into a single service layer.
- **ESPN Client**: Partial implementations of ESPN scraping logic are scattered between `src/lib/espn` and some older route files.
